...
try:
    from ..enhanced_patterns.base_analyzer import language_registry
    from ..language_analyzers.lua_analyzer import LuaAnalyzer
    MULTI_LANGUAGE_SUPPORT = True
except ImportError as e:
    logging.debug(f"Multi-language support not available: {e}")
...
class MemoryEnhancedRecursiveImprovement:
    ...
    def __init__(self, source_directory: Path):
        ...
        # Initialize multi-language analyzers
        self.language_analyzers = {}
        if MULTI_LANGUAGE_SUPPORT:
            try:
                # Register Lua analyzer
                lua_analyzer = LuaAnalyzer(self.memory)
                language_registry.register_analyzer(lua_analyzer)
                self.language_analyzers['lua'] = lua_analyzer
                self.logger.info("Multi-language support enabled with Lua analyzer")
            except Exception as e:
                self.logger.warning(f"Could not initialize language analyzers: {e}")
        else:
            self.logger.info("Multi-language support not available - Python analysis only")
    ...
    def analyze_code_file(self, file_path: Path):
        ...
        try:
            ...
            if MULTI_LANGUAGE_SUPPORT and language in self.language_analyzers:
                analyzer = self.language_analyzers[language]
                language_issues = analyzer.analyze_file(str(file_path))
                base_issues.extend(language_issues)
                self.logger.debug(f"Used {language} analyzer for {file_path.name}")
            else:
                # Fall back to generic Python analysis for unknown languages
                base_issues = self._detect_issues(content, file_path)
                self.logger.debug(f"Used generic analysis for {file_path.name} ({language})")
    ...
    def _find_target_files(self):
        """Find target files for analysis - supports multiple languages"""
        target_files = []
        ...
        # Find files for all supported languages
        all_files = []
        for language, patterns in file_patterns.items():
            if isinstance(patterns, str):
                patterns = [patterns]
            for pattern in patterns:
                found_files = list(self.source_directory.rglob(pattern))
                all_files.extend(found_files)
        
        self.logger.info(f"Found {len(all_files)} source files across multiple languages")
        ...
        self.logger.info(f"Found {len(target_files)} files for analysis:")
        for lang, count in language_counts.items():
            self.logger.info(f"  {lang}: {count} files")
